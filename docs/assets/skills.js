...
function render(data, verified = [], expanded = []) {
  ...
  const all = [...(data.records || []), ...expanded, ...verified].reduce((map, r) => { map.set(`${r.name}|${r.class}|${r.subcategory}`, r); return map; }, new Map());
...
Promise.all([
  ...
  fetch('data/skills-catalog.json', {cache:'no-store'}).then(r => r.json())
])
  .then(([data, verified, expanded]) => {
    window.__skillsData = data;
    window.__verifiedSkills = verified.records || [];
    window.__expandedSkills = expanded.records || [];
    render(data, window.__verifiedSkills, window.__expandedSkills);
  })
...
[q, type, sub].filter(Boolean).forEach(x => x.addEventListener('input', () => window.__skillsData && render(window.__skillsData, window.__verifiedSkills || [], window.__expandedSkills || [])));
