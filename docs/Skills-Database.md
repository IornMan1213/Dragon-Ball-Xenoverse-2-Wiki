# Skills Database (Expanded Local Reference)

This page keeps core skill information **on-wiki**. Full exhaustive tables live on Fandom; this wiki hosts categorized lists, acquisition methods, and notable examples so users can work without leaving.

## Skill Slot Structure
- 4 × Super Attacks
- 2 × Ultimate Attacks
- 1 × Evasive Skill
- 1 × Awoken Skill (transformation)

## Acquisition Methods (All Local)
| Method | Notes |
|--------|-------|
| Mentor Lessons | Lesson-specific rewards; exact mentor/lesson records are authoritative for the unlock |
| Parallel Quests | Skills may be listed as rewards; reward trigger, Ultimate Finish status, guarantee, and drop chance must be separately evidenced |
| TP Medal Shop | Rotating stock of skills and Super Souls |
| Shenron Wishes | Specific Super / Ultimate attacks via dedicated wishes |
| Crystal Raids / Double Crystal Raids | Broad skill pool |
| DLC Extra Missions & PQs | Exclusive skills per pack / Future Saga chapter |
| Events / Festival | Temporary or permanent rewards |

## Canonical acquisition boundary
For PQ skill acquisition, the canonical `pq_rewards_skill` relationship is the source of truth for the identity of the PQ→skill association. It does not, by itself, establish that the skill is guaranteed, requires an Ultimate Finish, comes from a particular enemy, or has a numerical drop rate. Those are research/evidence fields and require independent support.

## Festival of Universes — Named Skill Routes
The current repository evidence contains three explicitly named Festival skill routes. These are presented as Festival acquisitions rather than PQ relationships.

| Skill | Source character | Route |
|---|---|---|
| Galaxy Breaker (Festival) | Vegeta | Festival of Universes — Max Camaraderie with Vegeta |
| God Bind (Festival) | Goku (Super Saiyan God) | Festival of Universes — Max Camaraderie with Goku (Super Saiyan God) |
| Justice Crush (Festival) | Dyspo | Festival of Universes — Max Camaraderie with Dyspo |

**Evidence boundary:** the maintained Festival research batch establishes these three named records and their source characters/routes. It does **not** establish a complete Festival skill inventory, event scheduling/current availability, or unsupported PQ/raid relationships. See `docs/data/festival-skill-character-navigation-audit-2026-09-24.json` for the identity/navigation audit.

## Awoken Skills (Transformations)
**Universal / Multi-race**
- Kaioken (x1 / x3 / x20)
- Potential Unleashed (Super Class Advancement Test after Z-ranking prior tests)
- Beast
- Ultra Instinct (variants)
- The Power to Overcome (Future Saga Chapter 4 story)

**Saiyan**
- Super Saiyan 1 / 2 / 3
- Super Vegeta 1 / 2
- Future Super Saiyan
- Super Saiyan God
- Super Saiyan God Super Saiyan (Blue)
- Super Saiyan God Super Saiyan (Evolved)

**Race Exclusive**
- Power Pole Pro (Earthling)
- Become Giant (Namekian)
- Turn Golden (Frieza Race)
- Purification (Majin)

## Super Attacks – Notable Examples by Type
**Strike Supers** (close-range / physical / grabs): Orin Combo, Sledgehammer, Meteor Crash, Time Skip series (Hit), Spirit Stab, Savage Strike, Aura Slide, etc.

**Ki Blast Supers**: Kamehameha variants, Galick Gun, Destructo-Disc, Big Bang Attack, Death Beam, Consecutive Energy Blast, Dodon Ray, Tri-Beam, Masenko, Evil Explosion, Light Grenade, and many DLC additions.

**Charge / Utility**: Energy Charge, Full Power Charge, Maximum Charge, Ultimate Charge, Burst Charge, Rise to Action, Super Guard, Afterimage, etc.

## Ultimate Attacks – Notable Examples
**Strike Ultimates**: Dragon Fist, Unrelenting Barrage, Brave Sword Attack, various mentor and PQ rewards.

**Ki Blast Ultimates**: Super Kamehameha, Final Flash, Special Beam Cannon, Spirit Bomb variants, Minus Energy Power Ball, x100 Big Bang Kamehameha, and numerous DLC ultimates.

## Evasive Skills – Notable Examples
Spirit Explosion, Super Explosive Wave, Punisher Guard, Instant Rise, Super Back/Front Jump, Break Strike, various character-specific and DLC evasives (Science Vanish, Total Evasion, Evil Whirlwind, etc.).

**Sources**:
- dbxv2.fandom.com Super Attack, Ultimate Attack, Evasive Skill, Awoken Skill pages
- Mentor reward lists
- Community skill location videos and spreadsheets
- In-game Skill Shop and mentor menus

This page prioritizes usable local reference. Individual skill pages on Fandom remain the most detailed living source for exact unlock conditions of every single move.
