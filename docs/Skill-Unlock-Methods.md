# Skill Unlock Methods

Skills (Super, Ultimate, Evasive, Awoken) are obtained through multiple overlapping systems. The most reliable methods are listed below.

## Primary Methods
1. **Mentor Training** — Complete the four lessons of each available instructor. Where the lesson chain itself awards the skill, that mentor reward is a direct training reward; individual lesson records remain authoritative for the exact unlock.
2. **Parallel Quests** — Many skills are associated with specific PQ reward tables. Ultimate Finish conditions and reward triggers are separate evidence fields; consult the canonical PQ/skill records rather than assuming an Ultimate Finish guarantees a listed skill.
3. **TP Medal Shop** — Rotating stock of skills, Super Souls, and clothing. Check frequently (especially weekend rotations historically).
4. **Shenron Wishes** — Specific Super and Ultimate attacks via the corresponding wishes (limited uses / progressive).
5. **Crystal Raids / Double Crystal Raids** — Can drop a wide range of skills (including many that are otherwise PQ-locked). Useful for filling gaps.
6. **DLC / Future Saga Extra Missions & PQs** — Exclusive skills tied to each chapter.
7. **Events & Festivals** — Temporary or permanent skill rewards from Festival of Universes and login/campaign events; event availability is version-sensitive.

## Tips
- Prioritize mentor skills early — they are permanent and do not require grinding drops.
- For rare PQ skills, use community checklists or Fandom per-skill pages to identify the best quests.
- Partner Customization allows assigning unlocked skills to certain characters once the corresponding Key is obtained.

**Sources**:
- dbxv2.fandom.com skill and mentor pages
- dragonballxenoverse2.wiki skill and PQ guides
- Burcol and other “unlock every skill” video series
- Community Reddit spreadsheets and checklists
- Official DLC patch notes listing new moves
