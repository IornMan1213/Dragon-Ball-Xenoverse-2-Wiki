...
def main():
    ...
    duplicate_preset_ids=sorted(k for k,v in __import__("collections").Counter(r.get("id") for r in presets).items() if v>1)
    numbered_pairs=[(r.get("character_id"),r.get("preset_number")) for r in presets if r.get("preset_number") is not None]
    duplicate_character_preset_pairs=sorted([list(k) for k,v in __import__("collections").Counter(numbered_pairs).items() if v>1])
    special_records=[{"id":r.get("id"),"record_type":r.get("record_type"),"character_id":r.get("character_id")} for r in presets if r.get("record_type") and r.get("record_type")!="preset"]
