...
def main():
 ...
 rel=load("pq-reward-relationships.json")
 ...
 character_edges={(x.get("pq"), aliases.get(x.get("target"), x.get("target"))) for x in rel.get("verified_relationships",[]) if x.get("relationship")=="pq_features_character"}
 reverse_edges={(f"pq-{int(pq):03d}", name) for name,pqs in reverse.items() for pq in pqs}
 missing_reverse=sorted(character_edges-reverse_edges)
 extra_reverse=sorted(reverse_edges-character_edges)
 duplicate_reverse_pairs=[]
 ...
 checks={
  ...
  "reverse_exact_forward_parity":not missing_reverse and not extra_reverse,
  "reverse_duplicate_pairs_absent":not duplicate_reverse_pairs,
 ...
 out={"schema_version":"1.0.0","scope":"canonical character explorer consumer","canonical_characters":len(names),"preset_records":len(p["records"]),"preset_distinct_character_ids":len(preset_ids),"reverse_character_targets":len(reverse),"reverse_pq_references":sum(len(v) for v in reverse.values()),"unresolved_preset_ids":unresolved,"orphan_reverse_targets":orphan,"missing_reverse_pairs":missing_reverse,"extra_reverse_pairs":extra_reverse,"duplicate_reverse_pairs":duplicate_reverse_pairs,"checks":checks,"status":"clean" if all(checks.values()) else "unresolved","evidence_boundary":"Explorer navigation exposes canonical identity, indexed preset records, and explicit PQ reverse references only; it does not infer complete unlock routes, preset loadouts, raid rotation, DLC ownership, or drop mechanics."}
